...
## **Design:**
My design uses two files, one is a Client and one is a Server. The client allows the user to send in a list of usernames through a POST request to the server, which then will parse the incoming list and create a dictionary for each username. It will then use the GitHub API to search the usernames for their IDs and Public SSH Keys. If it finds the usernames ID and Key, it will add them into the dictionary it created for the username. Now some usernames will have more than one pair of IDs and Keys, so each dictionary created will have a list that holds the pairs of IDs and Keys just for this purpose. This way, you can see easily all of that usernames IDs and Public SSH Keys. One thing we can check is the status codes we get back when we try and send a GET request to the GitHub API. We have 3 checks at the end and will display a message accordingly. The first check is to see if we had a succefful GET request. So we check for a staus code of 200. This means everytihng went well. Another status code to check is 404, this means that the username we searched for could not be found. In this case, we would populate that usernames dictionary ID and Key as 'N/A'. The last thing we check for is 403. Now the reason we check for this is because there is a chance that you could use up all of your GitHub API calls. It may vary from person to person, however, I get 60 API calls per hour. So if I use 60 calls before an hour is up, I will then be unabe to get any data from the GitHub API and they code they return is 403. So if you look at the bottom we have a series of if statements and booleans dedicated to keeping track of what status codes we have run into.
